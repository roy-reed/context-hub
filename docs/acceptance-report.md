...
- 功能回归：21/21 通过，包括 Unicode 与哈希分页、FTS5/LIKE 分流、版本替换与
...
结果：21 个测试全部通过，用时 17.851 秒。覆盖的关键不变量包括：
...
| 检查项 | 实测 | 目标 | 结果 |
| --- | ---: | ---: | --- |
| 核心搜索 p50（200 次） | 2.722 ms | <= 50 ms | 通过 |
| 核心搜索 p95（200 次） | 3.417 ms | <= 150 ms | 通过 |
| 预热 MCP 搜索 p95（40 次） | 4.626 ms | <= 500 ms | 通过 |
| STDIO 启动并完成 `tools/list`（3 次中位数） | 544.963 ms | <= 1000 ms | 通过 |
| `doctor` 精确计数 | 10,000 / 10,000 | 相等 | 通过 |

补充测量：三次独立的 STDIO 冷启动为 633.023、544.963、536.138 ms；完整重建
10,000 条索引用时 345.043 ms，核心首次搜索 4.473 ms。
